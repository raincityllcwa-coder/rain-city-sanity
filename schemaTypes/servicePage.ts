...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      description: 'The blocks under the hero (with the form strip), top to bottom. Drag a row by the handle on its left to move it, pick the block in the dropdown, Add item adds a row, the row menu (...) removes one. Empty list = the default order. A block that is not in the list is not shown, except the SEO text: when it is not listed it stays at the bottom.',
